...
export async function pgRequest(path, { method = 'GET', body, prefer } = {}) {
  ...
  // Inserts/updates without `Prefer: return=representation` come back with
  // an empty body (sometimes status 201, not just 204) — calling .json() on
  // that throws a SyntaxError, so check for empty text first.
  const text = await res.text();
  if (!text) return null;
